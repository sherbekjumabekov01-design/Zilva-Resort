...
export default function RoomCard({ room }: { room: Room }) {
  ...
  return (
    <div className="group glass-card glass-card-interactive rounded-3xl overflow-hidden shadow-lg transition-all duration-300 flex flex-col">
      {/* Image Container */}
      <div className="relative h-64 sm:h-72 w-full overflow-hidden bg-neutral-900">
        ...
        <div className="absolute bottom-4 left-4 right-4 flex items-center gap-3 text-white text-xs font-medium">
          ...
          <div className="flex items-center gap-1.5 bg-black/40 backdrop-blur-md px-2.5 py-1 rounded-lg">
            ...
            <span>{room.maxGuests} mehmongacha</span>
          </div>
          {hasJacuzzi && (
      ...
      <div className="p-6 flex-1 flex flex-col justify-between space-y-4">
        <div className="space-y-2">
          ...
          <h3 className="font-serif text-xl sm:text-2xl font-bold text-[#18221b] group-hover:text-[#1b382b] transition-colors line-clamp-1">
            <Link href={`/rooms/${room.slug}`}>
              {room.name}
            </Link>
          </h3>

          <p className="text-xs sm:text-sm text-[#5e6962] line-clamp-2 leading-relaxed font-light">
            {room.shortDescription}
          </p>
        </div>

        {/* Specs & Amenities Pills */}
        <div className="pt-3 border-t border-[#dfd8cb]/60 flex flex-wrap gap-1.5">
          <span className="text-[11px] px-2.5 py-1 rounded-md bg-[#f0ebe1] text-[#18221b] flex items-center gap-1 font-medium">
            <Bed className="w-3 h-3 text-[#b88a44]" />
            <span>{room.bedType}</span>
          </span>
          {room.breakfastIncluded && (
            <span className="text-[11px] px-2.5 py-1 rounded-md bg-emerald-50 text-emerald-800 border border-emerald-200 font-medium">
              Nonushta kiritilgan
            </span>
          )}
        </div>
        ...
        <div className="pt-4 border-t border-[#dfd8cb] flex items-center justify-between gap-3">
          <div>
            <span className="block text-[11px] text-[#5e6962] uppercase tracking-wider">Boshlang'ich narx</span>
            <div className="text-lg sm:text-xl font-serif font-bold text-[#1b382b]">
          ...
          <div className="flex items-center gap-2">
            <Link
              ...
              className="p-2.5 rounded-xl bg-[#f0ebe1] hover:bg-[#dfd8cb] text-[#18221b] transition-colors"
